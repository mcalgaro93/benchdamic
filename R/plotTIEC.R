...
createTIEC <- function(object){
    ...
    df_list_pval <- lapply(X = object, FUN = function(Comparison){
        plyr::ldply(Comparison, function(method){
            ...
            if(!is.element("pValMat", names(method)))
                stop("'pValMat' matrix not found for one of the methods in
                    input.")
    ...
    df_KS <- plyr::ddply(.data = df_QQ_KS, .variables = ~ Comparison + Method,
        .fun = function(x){ colMeans(x[, c("KS", "KS_pval")]) })
    return(list(df_pval = df_pval, df_FPR = df_FPR, df_QQ = df_QQ, df_KS =
        df_KS))
...
#' cond <- factor(c("A","A","B","B","C","D"), levels = c("A","B","C","D"))
#'
...
plotFPR <- function(df_FPR, cols = NULL){
    ...
    ord001 <- order(plyr::ddply(.data = df_FPR, .variables = ~ Method, .fun =
        function(x) mean(x$FPR_obs001))$V1)
    ord005 <- order(plyr::ddply(.data = df_FPR, .variables = ~ Method, .fun =
        function(x) mean(x$FPR_obs005))$V1)
    ord01 <- order(plyr::ddply(.data = df_FPR, .variables = ~ Method, .fun =
    ...
    g <- ggplot2::ggplot(data = df_FPR, ggplot2::aes(color = Method)) +
        ggplot2::geom_boxplot(data = df_FPR001, ggplot2::aes(x = "0.01", y =
            FPR_obs001)) +
        ggplot2::geom_boxplot(data = df_FPR005, ggplot2::aes(x = "0.05",y =
            FPR_obs005)) +
        ggplot2::geom_boxplot(data = df_FPR01, ggplot2::aes(x = "0.1",y =
            FPR_obs01)) +
        ggplot2::geom_segment(ggplot2::aes(x = 1 - 0.5, xend = 1 + 0.5, y =
            0.01, yend = 0.01), color = "red", lty = 2) +
        ggplot2::geom_segment(ggplot2::aes(x = 2 - 0.5, xend = 2 + 0.5, y =
            0.05, yend = 0.05), color = "red", lty = 2) +
        ggplot2::geom_segment(ggplot2::aes(x = 3 - 0.5, xend = 3 + 0.5, y = 0.1,
            yend = 0.1), color = "red", lty = 2) +
        ggplot2::xlab(expression(Nominal ~ alpha)) +
        ggplot2::ylab(expression(Observed ~ alpha)) +
        ggplot2::ggtitle(label = "False Positive Rate", subtitle =
            "For each method") +
...
plotQQ <- function(df_QQ, cols = NULL, zoom = c(0,0.1)){
    ...
    df_segments <- data.frame(x1 = c(0.01, 0, 0.05, 0, 0.1, 0), x2 = c(0.01,
        0.01, 0.05, 0.05, 0.1, 0.1), y1 = c(0, 0.01, 0, 0.05, 0, 0.1), y2 = c(
    ...
    g <- ggplot2::ggplot(data = df_QQ, ggplot2::aes(x = pval_theoretical,
        ...
        ggplot2::ggtitle(label = "Average QQ-plot", subtitle = paste("From",
            zoom[1], "to", zoom[2])) +
        ggplot2::geom_segment(ggplot2::aes(x = x1, xend = x2, y = y1,
            yend = y2), color = "red", lty = 2, data = df_segments) +
        ggplot2::scale_color_manual(values = cols)
    return(g)
}
...
plotKS <- function(df_KS, cols = NULL){
    ...
    if(is.null(names(cols)))
        stop("'cols' vector is not a named vector of colors.")
    ord <- order(plyr::ddply(.data = df_KS, .variables = ~ Method, .fun =
        function(x) mean(x$KS))$V1)
    df_KS$Method <- factor(df_KS$Method, levels = levels(df_KS$Method)[ord],
        ordered = TRUE)
    # to avoid notes during the check
    Method <- KS <- NULL
    g <- ggplot2::ggplot(df_KS, ggplot2::aes(x = Method,y = KS, color =
        Method)) +
        ...
        ggplot2::ggtitle(label = "K-S statistics", subtitle =
            "Ordered methods") +
        ggplot2::scale_x_discrete(limits = rev(levels(df_KS$Method)), position =
            "top") +
        ggplot2::theme(legend.position = "none", panel.spacing = unit(0,
            "lines"), plot.margin = unit(c(0,0,0,1), "cm")) +
        ggplot2::scale_color_manual(values = cols)
    return(g)
}
