...
-   `corncob` (temporarily removed) with a focus on average differences (not dispersion, regulated by phi.formula and phi.formula_null parameters) using both *Wald* and *LRT* tests [@corncob];
...
my_methods <- c(my_basic, my_edgeR, my_DESeq2, my_limma, my_metagenomeSeq,
    # my_corncob, 
    my_ALDEx2, my_MAST, my_Seurat, my_ANCOM, my_dearseq, my_NOISeq)
```
...
From this example, only **8** out of **23** methods are able to find an enriched amount of truly DA features without any false discovery: `ANCOM.counts.BC` and `dearseq.counts.permutation.1000` in the first position.
