...
Firstly, the microbial metabolism information is necessary. These data comes from [@cigarettes] paper github repository ([https://github.com/waldronlab/nychanesmicrobiome](https://github.com/waldronlab/nychanesmicrobiome)), but they can be loaded using `data("microbial_metabolism")`.
...
`Plaque_16_DA` object contains the results for 9 methods. In order to extract p-values and directionality of DA (Over abundant or Under abundant) the `extractStatistics()` function can be used, defining the *direction* argument.
...
In the *direction* argument, the column name containing the direction (e.g. *logfc*, *logFC*, *logFoldChange*...) of each method's `statInfo` matrix must be supplied.
